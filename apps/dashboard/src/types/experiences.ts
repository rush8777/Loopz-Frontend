...
export interface BuilderCanvasViewport { zoom: number; panX: number; panY: number }
export interface WidgetBuilderState { version: 1; projectData: Record<string, unknown>; html: string; css: string; canvas?: BuilderCanvasViewport }
export type ExperienceLayer = { mode: "auto" } | { mode: "relative"; relation: "above" | "below"; target: ExperienceTarget } | { mode: "always_on_top" } | { mode: "custom"; zIndex: number };
export interface ExperienceBehavior { dismissible: boolean; layer?: ExperienceLayer; zIndex?: number; placement?: "auto" | "top" | "right" | "bottom" | "left"; alignment?: "start" | "center" | "end"; offset?: number; pointer?: { enabled?: boolean; size?: number }; toastPosition?: "top-left" | "top-right" | "bottom-left" | "bottom-right"; autoDismissMs?: number | null; cursorOffset?: { x: number; y: number }; modalLayout?: "center" | "fullscreen"; backdrop?: boolean; backdropOpacity?: number; closeOnBackdrop?: boolean; slideoutPosition?: "top-left" | "top-right" | "bottom-left" | "bottom-right" | "center-left" | "center-right"; bannerPosition?: "top" | "bottom"; hotspotStyle?: "pulse" | "dot" | "question"; hotspotColor?: string }
...
export type ChecklistItemAction = { type: "launch_guide"; experienceId: string } | { type: "navigate"; url: string } | { type: "open_url"; url: string } | { type: "none" };
export type ChecklistItemCompletion = { type: "segment"; segmentId: string } | { type: "guide_completed"; experienceId: string } | { type: "item_clicked" };
export interface ChecklistItem { id: string; title: string; description?: string; action: ChecklistItemAction; completion: ChecklistItemCompletion }
...
export interface ExperienceAnalytics {
  ...
  checklist?: null | { usersShown: number; usersOpened: number; usersStarted: number; usersCompleted: number; completionRate: number; usersDismissed: number; items: Array<{ itemId: string; title: string; index: number; uniqueClicks: number; uniqueCompletions: number; completionRate: number }> };
...
export function isChecklistDefinition(definition: ExperienceDefinition): definition is ChecklistExperienceDefinition { return "items" in definition; }
export function isWidgetDefinition(definition: ExperienceDefinition): definition is Extract<ExperienceDefinition, { content: ExperienceContent }> { return "content" in definition; }
