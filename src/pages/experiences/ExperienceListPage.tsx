...
export function ExperienceListPage({ kind, widgetType, singularLabel = kind, pluralLabel = kind === "guide" ? "Guides" : "Widgets", description, unsupported = false }: ExperienceListPageProps) {
  ...
  async function preview(item: Experience) { if (!currentOrg || !currentSite) return; const popup = openLiveEditorPopup(); if (!popup) { setError("Your browser blocked the live editor popup. Please allow popups for Movcues and try again."); return; } try { const session = await experiencesApi.createEditorSession(currentOrg.orgId, currentSite.id, item.id); popup.location.replace(session.launchUrl); popup.focus(); } catch (caught) { popup.close(); setError(caught instanceof Error ? caught.message : "Preview failed."); } }
