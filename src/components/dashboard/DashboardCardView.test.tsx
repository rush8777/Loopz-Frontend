...
describe("DashboardCardView", () => {
  it("renders scalar comparison, compact footer, and drill-down", () => { const onDrilldown = vi.fn(); render(<DashboardCardView card={card} filters={filters} response={response} onDrilldown={onDrilldown} />); expect(screen.getByText("120")).toBeInTheDocument(); expect(screen.getByText("+20% vs previous")).toBeInTheDocument(); expect(screen.getByText(/Updated /)).toBeInTheDocument(); expect(screen.queryByText(/day.*UTC/)).not.toBeInTheDocument(); fireEvent.click(screen.getByRole("button", { name: "View details" })); expect(onDrilldown).toHaveBeenCalled(); });
