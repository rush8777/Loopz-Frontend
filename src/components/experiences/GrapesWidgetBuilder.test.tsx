...
describe("GrapesWidgetBuilder", () => {
  ...
  it("initializes once, mounts custom managers, bootstraps once, and destroys cleanly", async () => {
    vi.useFakeTimers(); const editor = fakeEditor(); const onChange = vi.fn(); const props = { experienceKey: "exp:v1", widgetType: "modal" as const, content: { heading: "Hello", body: "World" }, design: { width: "md" as const, theme: { background: "#fff", foreground: "#111", primary: "#2563eb", borderRadius: "md" as const } }, onChange, onPrimaryActionChange: vi.fn() };
  ...
  it("exports only dirty project mutations after the builder debounce", async () => {
    vi.useFakeTimers(); fakeEditor(); const onChange = vi.fn(); render(<GrapesWidgetBuilder experienceKey="exp:v2" widgetType="banner" value={{ version: 1, projectData: { pages: [] }, html: '<section class="loopz-widget"></section>', css: ".loopz-widget{}" }} content={{ heading: "Hello", body: "World" }} design={{ width: "md", theme: { background: "#fff", foreground: "#111", primary: "#2563eb", borderRadius: "md" } }} onChange={onChange} onPrimaryActionChange={vi.fn()} />);
    await act(async () => { await vi.dynamicImportSettled(); vi.runOnlyPendingTimers(); }); expect(onChange).not.toHaveBeenCalled(); expect(harness.init.mock.calls[0][0]).not.toHaveProperty("projectData"); expect(harness.init.mock.calls[0][0].components).toContain("loopz-widget"); harness.dirty = 1; act(() => harness.handlers.get("update")?.()); act(() => vi.advanceTimersByTime(400)); expect(onChange).toHaveBeenCalledTimes(1);
  });

  it("applies sanitized HTML and scoped CSS from code mode exactly once", async () => {
    vi.useFakeTimers(); const editor = fakeEditor(); const onChange = vi.fn(); render(<GrapesWidgetBuilder experienceKey="exp:v3" widgetType="toast" value={{ version: 1, projectData: { pages: [] }, html: '<section class="loopz-widget"></section>', css: ".loopz-widget{}" }} content={{ heading: "Hello", body: "World" }} design={{ width: "md", theme: { background: "#fff", foreground: "#111", primary: "#2563eb", borderRadius: "md" } }} onChange={onChange} onPrimaryActionChange={vi.fn()} />);
  ...
  it("flushes a pending canvas mutation without waiting for the debounce", async () => {
    vi.useFakeTimers(); fakeEditor(); const onChange = vi.fn(); const ref = createRef<GrapesWidgetBuilderHandle>(); render(<GrapesWidgetBuilder ref={ref} experienceKey="exp:v4" widgetType="modal" value={{ version: 1, projectData: { original: true }, html: '<section class="loopz-widget"></section>', css: ".loopz-widget{}" }} content={{ heading: "Hello", body: "World" }} design={{ width: "md", theme: { background: "#fff", foreground: "#111", primary: "#2563eb", borderRadius: "md" } }} onChange={onChange} onPrimaryActionChange={vi.fn()} />);
  ...
  it("uses a neutral authoring surface and fits the widget inside the visible workspace", async () => {
    vi.useFakeTimers(); const editor = fakeEditor(); render(<GrapesWidgetBuilder experienceKey="exp:v5" widgetType="modal" value={{ version: 1, projectData: { pages: [] }, html: '<section class="loopz-widget"></section>', css: ".loopz-widget{}" }} content={{ heading: "Hello", body: "World" }} design={{ width: "md", theme: { background: "#fff", foreground: "#111", primary: "#2563eb", borderRadius: "md" } }} onChange={vi.fn()} onPrimaryActionChange={vi.fn()} />);
