...
function fakeEditor() {
  let html = '<section class="movecues-widget"><h2 data-movecues-content="heading">Hello</h2><p data-movecues-content="body">World</p></section>'; let css = ".movecues-widget{color:#111}"; let device = "Desktop"; const widgetElement = document.createElement("section"); const canvasDocument = document.implementation.createHTMLDocument(); const frameElement = document.createElement("iframe");
  const editor: any = {
    DomComponents: { addType: vi.fn() }, UndoManager: { undo: vi.fn(), redo: vi.fn() }, Canvas: { setZoom: vi.fn(), setCoords: vi.fn(), getDocument: vi.fn(() => canvasDocument), getFrameEl: vi.fn(() => frameElement) }, refresh: vi.fn(),
