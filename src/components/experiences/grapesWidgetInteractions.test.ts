...
function componentFixture({ classes = [], tagName = "div", style = {}, parent }: { classes?: string[]; tagName?: string; style?: Record<string, string>; parent?: Component } = {}) {
  ...
  const component = {
    ...
    getAttributes: () => ({ "data-loopz-action-id": "primary" }),
...
describe("GrapesJS widget interactions", () => {
  it("assigns capability-specific resize handles to images, containers, and text", () => {
    ...
    const container = componentFixture({ classes: ["loopz-widget__container"] }).component;
    const text = componentFixture({ classes: ["loopz-widget__body"], tagName: "p" }).component;
  ...
  it("routes root width and height through existing widget clamps", () => {
    const root = componentFixture({ classes: ["loopz-widget"] }).component;
  ...
  it("enables and safely restores free positioning without changing action attributes", () => {
    ...
    const button = componentFixture({ classes: ["loopz-widget__button"], tagName: "button", style: { color: "red" }, parent: parentFixture.component });
    setFreePosition(button.component, true);
    expect(button.dragMode()).toBe("absolute"); expect(button.style()).toMatchObject({ color: "red", position: "absolute", left: "12px", top: "20px" }); expect(parentFixture.style().position).toBe("relative"); expect(button.component.getAttributes()["data-loopz-action-id"]).toBe("primary");
    setFreePosition(button.component, false);
    expect(button.dragMode()).toBe(""); expect(button.style()).toEqual({ color: "red" }); expect(parentFixture.style().position).toBeUndefined(); expect(button.component.getAttributes()["data-loopz-action-id"]).toBe("primary");
  });
  ...
  it("adds Free position while leaving normal responsive dragging as the default", () => {
    ...
    const button = componentFixture({ classes: ["loopz-widget__button"], tagName: "button" });
    ...
    expect(button.values.get("resizable")).toMatchObject({ tl: true, br: true }); expect(button.traits).toContainEqual(expect.objectContaining({ name: "loopzFreePosition" })); expect(button.dragMode()).toBe("");
  });
});
