...
export const GrapesWidgetBuilder = forwardRef<GrapesWidgetBuilderHandle, Props>(function GrapesWidgetBuilder({ experienceKey, widgetType, value, content, design, onChange, onPrimaryActionChange }, ref) {
  ...
  const [ready, setReady] = useState(false); const [device, setDevice] = useState("Desktop"); const [codeMode, setCodeMode] = useState(false); const [codeHtml, setCodeHtml] = useState(value?.html ?? ""); const [codeCss, setCodeCss] = useState(value?.css ?? ""); const [codeError, setCodeError] = useState<string | null>(null); const [selectedAction, setSelectedAction] = useState<"primary" | "secondary" | null>(null);
  ...
  useEffect(() => {
    let cancelled = false; let timer = 0; let editor: Editor | null = null; let initialized = false; let resizeObserver: ResizeObserver | null = null;
    ...
    const schedule = () => { if (!editor || applyingCodeRef.current || editor.getDirtyCount() === 0) return; clearTimeout(timer); timer = window.setTimeout(() => { emit(); fitCanvasRef.current(); }, 400); };
    flushExportRef.current = () => { clearTimeout(timer); emit(); };
    const finishInitialization = () => {
      if (!editor || initialized || cancelled) return; initialized = true; setReady(true);
    ...
    void import("grapesjs").then(module => {
      ...
      editor = grapesjs.init({
        ...
        deviceManager: { devices: [{ id: "desktop", name: "Desktop", width: "1200px", height: "900px" }, { id: "tablet", name: "Tablet", width: "1200px", height: "900px", widthMedia: "768px" }, { id: "mobile", name: "Mobile", width: "1200px", height: "900px", widthMedia: "390px" }] },
      ...
      fitCanvasRef.current = () => {
        ...
        editor.Canvas.fitViewport({
          el: widget,
          gap: 40,
          zoom: zoom => Math.min(100, Math.max(10, zoom)),
        });
        const { x } = editor.Canvas.getCoords();
        editor.Canvas.setCoords(x, 0);
      };
      if (typeof ResizeObserver !== "undefined" && canvasRef.current) {
        resizeObserver = new ResizeObserver(() => fitCanvasRef.current());
    ...
    return () => { cancelled = true; clearTimeout(timer); resizeObserver?.disconnect(); flushExportRef.current = () => void 0; fitCanvasRef.current = () => void 0; editorRef.current = null; editor?.destroy(); };
  ...
  const chooseDevice = (name: string) => { editorRef.current?.setDevice(name); setDevice(name); fitCanvasRef.current(); };
