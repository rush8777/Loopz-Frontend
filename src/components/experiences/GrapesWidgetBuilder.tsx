...
import { FREE_LAYOUT_ROOT_CLASS, installWidgetInteractions, supportsFreeLayout, type FreeItemBox, type WidgetInteractionController } from "./grapesWidgetInteractions";
...
export const GrapesWidgetBuilder = forwardRef<GrapesWidgetBuilderHandle, Props>(function GrapesWidgetBuilder({ experienceKey, widgetType, value, content, design, onChange, onPrimaryActionChange, onSizeChange }, ref) {
  ...
  useEffect(() => {
    ...
    setPositioned(false);
    viewportRef.current = { zoom: 100, panX: 0, panY: 0 }; setZoomLabel(100); setHandTool(false); setSpacePressed(false); setPanning(false); setFreeLayout(Boolean(value?.html.includes(FREE_LAYOUT_ROOT_CLASS))); setFreeItemBox(null); spacePressedRef.current = false; panGestureRef.current = null; selectedFreeItemRef.current = null;
    ...
    const finishInitialization = () => {
      ...
      interactionControllerRef.current?.syncLayoutFromRoot();
    ...
    void import("grapesjs").then(module => {
      ...
      editor = grapesjs.init({
        ...
        plugins: [instance => instance.DomComponents.addType("movecues-button", { isComponent: element => element.tagName === "BUTTON" && element.hasAttribute("data-movecues-action-id") ? { type: "movecues-button" } : false, model: { defaults: { tagName: "button", droppable: false, editable: true, traits: [] } } }), instance => { interactionControllerRef.current = installWidgetInteractions(instance, { widgetType, design: () => designRef.current, onRootResize: (size, commit) => { const next = { ...designRef.current, size }; applySizeEnvelopeRef.current(next); if (commit) { designRef.current = next; onSizeChangeRef.current(size); } }, onFreeLayoutChange: setFreeLayout, onFreeItemChange: (component, box) => { selectedFreeItemRef.current = component; setFreeItemBox(box); } }); }],
      });
