...
export const GrapesWidgetBuilder = forwardRef<GrapesWidgetBuilderHandle, Props>(function GrapesWidgetBuilder({ experienceKey, widgetType, value, content, design, onChange, onPrimaryActionChange, onSizeChange }, ref) {
  ...
  useEffect(() => {
    ...
    setPositioned(false);
    viewportRef.current = { zoom: 100, panX: 0, panY: 0 }; setZoomLabel(100); setHandTool(false); setSpacePressed(false); setPanning(false); spacePressedRef.current = false; panGestureRef.current = null;
    ...
    void import("grapesjs").then(module => {
      ...
      editor = grapesjs.init({
        ...
        plugins: [instance => instance.DomComponents.addType("movecues-button", { isComponent: element => element.tagName === "BUTTON" && element.hasAttribute("data-movecues-action-id") ? { type: "movecues-button" } : false, model: { defaults: { tagName: "button", droppable: false, editable: true, traits: [] } } }), instance => installWidgetInteractions(instance, { widgetType, design: () => designRef.current, onRootResize: (size, commit) => { const next = { ...designRef.current, size }; applySizeEnvelopeRef.current(next); if (commit) { designRef.current = next; onSizeChangeRef.current(size); } } })],
      });
      ...
      const selectAction = (component?: Component) => { const slot = component?.getAttributes()?.["data-movecues-action-id"]; setSelectedAction(slot === "primary" || slot === "secondary" ? slot : null); setSidebarTab("properties"); };
    ...
    return () => {
      ...
      flushExportRef.current = () => void 0; fitCanvasRef.current = () => void 0; scheduleCanvasFitRef.current = () => void 0; scheduleCanvasRefreshRef.current = () => void 0; applyViewportRef.current = () => void 0; applySizeEnvelopeRef.current = () => void 0; editorRef.current = null; editor?.destroy();
    };
